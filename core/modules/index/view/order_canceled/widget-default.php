	...
	<div class="col-md-12">
		...
		if(isset($_GET["start_at"]) && $_GET["start_at"]!="" && isset($_GET["finish_at"]) && $_GET["finish_at"]!=""){
			...
			if($_GET["start_at"]<$_GET["finish_at"]){
				$products = OperationData::getRentsByRangeCanceled($_GET["start_at"],$_GET["finish_at"]);
			}
		}
		else{

			$products = OperationData::getRentsCanceled();
		}
		if(count($products)>0){
		?>
			<br>
			...
			?>
			<p class="alert alert-danger">No hay devoluciones en estos momentos.</p>
			<?php
		}
		?>
