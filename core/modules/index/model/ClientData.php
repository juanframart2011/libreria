...
class ClientData {
	...
	public function add(){
		...
		$sql = "insert into ".self::$tablename." (user_id, name,lastname,address,phone,email,created_at) ";
		$sql .= "value (\"$this->user\",\"$this->name\",\"$this->lastname\",\"$this->address\",\"$this->phone\",\"$this->email\",$this->created_at)";
	...
	public function updateInventory( $inventory, $cliente ){
		$sql = "update ".self::$tablename." set initial_inventory=\"$inventory\" where id=$cliente";
		Executor::doit($sql);
	}
