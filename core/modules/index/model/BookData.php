...
class BookData {
	...
	public function add(){
		$sql = "insert into book (isbn,title,subtitle,description,cantidad,precio_costo,n_pag,year,category_id,editorial_id,author_id) ";
		$sql .= "value (\"$this->isbn\",\"$this->title\",\"$this->subtitle\",\"$this->description\",\"$this->cantidad\",\"$this->precio_costo\",\"$this->n_pag\",\"$this->year\",$this->category_id,$this->editorial_id,$this->author_id)";
...
// partiendo de que ya tenemos creado un objecto BookData previamente utilizamos el contexto
	public function update(){
		...
		$sql = "update ".self::$tablename." set title=\"$this->title\",subtitle=\"$this->subtitle\",cantidad=$this->cantidad,precio_costo=$this->precio_costo,isbn=\"$this->isbn\",description=\"$this->description\",n_pag=\"$this->n_pag\",year=\"$this->year\",category_id=$this->category_id,editorial_id=$this->editorial_id,author_id=$this->author_id where id=$this->id";
