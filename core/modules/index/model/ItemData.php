...
class ItemData {
	...
	public function add(){
		$sql = "insert into item (code,cantidad,status_id,book_id) ";
		$sql .= "value (\"$this->code\",\"$this->cantidad\",\"$this->status_id\",\"$this->book_id\")";
...
// partiendo de que ya tenemos creado un objecto ItemData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set code=\"$this->code\",cantidad=\"$this->cantidad\",status_id=\"$this->status_id\" where id=$this->id";
		Executor::doit($sql);
	}
	...
	public static function countAvaiableByBookId($id){
		$sql = "select sum(cantidad) as c from ".self::$tablename." where book_id=$id and status_id=1";
		$query = Executor::doit($sql);
		return Model::one($query[0],new ItemData());
	}
